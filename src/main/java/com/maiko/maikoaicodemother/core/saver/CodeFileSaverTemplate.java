...
public abstract class CodeFileSaverTemplate<T> {
    ...
    /**
     * 模板方法：保存代码的标准流程
     * M：它这个加final是因为流程是不能被重写的，模板方法流程是固定的。
     *
     * @param result 代码结果对象
     * @return 保存的目录
     */
    ...
    /**
     * 验证输入参数（可由子类覆盖）
     * protected只能子类实现这个方法，所以用protected修饰
     *
     * @param result 代码结果对象
     */
    ...
    /**
     * 写入单个文件的工具方法
     * M：它为什么在这里加了一个这个方法，因为子类saveFiles需要调用writeToFile方法
     * 那怎么调用呢？写在公共的父类中，这样子类就可以调用了
     * 父类提供的通用方法不允许子类去复写所以用了final
     *
     * @param dirPath  目录路径
     * @param filename 文件名
     * @param content  文件内容
